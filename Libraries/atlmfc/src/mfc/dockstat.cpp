	...
	if (ar.IsStoring())
	{
		...
		if (m_arrBarID.GetSize() != 0)
		{
			ar.Write(&m_arrBarID.ElementAt(0),
				(ULONG)(m_arrBarID.GetSize()*sizeof(DWORD)));
		}
	}
	else
	{
...
BOOL CControlBarInfo::SaveState(LPCTSTR lpszProfileName, int nIndex)
{
	...
	if (m_arrBarID.GetSize() > 1) //if ==1 then still empty
	{
		...
		for (int i = 0; i < m_arrBarID.GetSize(); i++)
		{
			TCHAR buf[16];
			_stprintf_s(buf, _countof(buf), _afxBar, i);
			pApp->WriteProfileInt(szSection, buf, (int)(INT_PTR)m_arrBarID[i]);
		}
	}
	return TRUE;
}
...
void CDockBar::GetBarInfo(CControlBarInfo* pInfo)
{
	...
	for (int i = 0; i < m_arrBars.GetSize(); i++)
	{
		...
		if (pBar != NULL && (DWORD_PTR(pBar) <= 0xffff))
		{
			...
			void* pRememberedID = (void *)(LONG_PTR)MAKELONG(w, 1);
			pInfo->m_arrBarID.Add(pRememberedID);
		}
		else
		{
			pInfo->m_arrBarID.Add(pBar == NULL ?
				0 : (void*)(DWORD_PTR)_AfxGetDlgCtrlID(pBar->m_hWnd));
		}
	}
}
...
void CDockBar::SetBarInfo(CControlBarInfo* pInfo, CFrameWnd* pFrameWnd)
{
	...
	while (nSize != 0 && (pInfo->m_arrBarID[nSize-1] == NULL ||
		pInfo->m_arrBarID[nSize-1] == (void*)(LONG_PTR)MAKELONG(0, 1)))
	{
		nSize--;
	}
	...
	for (int i = 1; i < nSize; i++)
	{
		...
		if (DWORD_PTR(pInfo->m_arrBarID[i]) <= 0xffff)
		{
			pBar = pFrameWnd->GetControlBar((UINT)(UINT_PTR)pInfo->m_arrBarID[i]);
			if (pBar != NULL)
			{
		...
		{
			WORD w = LOWORD(((DWORD_PTR)pInfo->m_arrBarID[i]));
