...
{
inline int RFX_Text_strlen(__in LPSTR  psz){return ::lstrlenA(psz);}
inline int RFX_Text_strlen(__in LPWSTR psz){return ::lstrlenW(psz);}
...
inline void RFX_Text_inc(LPSTR *ppsz)
		...
		{(*ppsz)++;}
	#endif
inline void RFX_Text_inc(__inout LPWSTR *ppsz){(*ppsz)++;}
	...
	case AFX_RFX_BINARY:
		{
			...
			return nSize == pByteArrayDest->GetSize() &&
				   memcmp(&pByteArrayDest[0], &pByteArraySrc[0], nSize) == 0;
		}
...
inline void RFX_Text_Bulk(CFieldExchange* pFX, LPCTSTR szName,
	__deref_out_ecount(nMaxLength) CharType **prgStrVals, LONG_PTR** prgLengths, int nMaxLength,
	short nFieldType)
{
	ENSURE_ARG(AfxIsValidAddress(pFX, sizeof(CFieldExchange)));
	ENSURE_ARG(AfxIsValidString(szName));
...
void AFXAPI RFX_Text_Bulk(CFieldExchange* pFX, LPCTSTR szName,
	__deref_out_ecount(nMaxLength) LPWSTR *prgStrVals, LONG_PTR** prgLengths, int nMaxLength)
{
	RFX_Text_Bulk(pFX, szName, prgStrVals, prgLengths, nMaxLength, SQL_C_WCHAR);
}

void AFXAPI RFX_Text_Bulk(CFieldExchange* pFX, LPCTSTR szName,
	__deref_out_ecount(nMaxLength) LPSTR *prgStrVals, LONG_PTR** prgLengths, int nMaxLength)
{
