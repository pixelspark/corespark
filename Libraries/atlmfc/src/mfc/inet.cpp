...
#define _AFX_TRANSFER_MASK (INTERNET_FLAG_TRANSFER_BINARY | INTERNET_FLAG_TRANSFER_ASCII)
	...
	if (bParsed && dwServiceType == AFX_INET_SERVICE_FILE)
	{
		...
		pResult = new CStdioFile(strObject, nMode);
	}
	else
	{
		HINTERNET hOpener;
...
BOOL CInternetSession::GetCookie(LPCTSTR pstrUrl, LPCTSTR pstrCookieName, __out_ecount(dwBufLen) LPTSTR pstrCookieData, DWORD dwBufLen)
{
	ASSERT(AfxIsValidString(pstrUrl));
	ASSERT(AfxIsValidString(pstrCookieName));
...
LPTSTR CInternetFile::ReadString(__out_ecount(nMax) LPTSTR pstr, UINT nMax)
{
	ASSERT_VALID(this);
	ASSERT(m_hFile != NULL);
...
BOOL CFtpConnection::GetCurrentDirectory(__out_ecount_part(*lpdwLen, *lpdwLen) LPTSTR pstrDirName,
	__inout LPDWORD lpdwLen) const
{
	ASSERT_VALID(this);
	ASSERT(m_hConnection != NULL);
...
BOOL CFtpConnection::GetCurrentDirectoryAsURL(__out_ecount_part(*lpdwLen, *lpdwLen) LPTSTR pstrName,
	__inout LPDWORD lpdwLen) const
{
	ASSERT(lpdwLen != NULL);
...
BOOL CInternetException::GetErrorMessage(__out_ecount(nMaxError) LPTSTR pstrError, UINT nMaxError,
