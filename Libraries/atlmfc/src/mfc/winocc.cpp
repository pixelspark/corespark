...
int CWnd::GetDlgItemText(int nID, __out_ecount(nMaxCount) LPTSTR lpStr, int nMaxCount) const
{
	ASSERT(::IsWindow(m_hWnd));
...
int CWnd::GetWindowText(__out_ecount(nMaxCount) LPTSTR lpszString, int nMaxCount) const
{
	ASSERT(::IsWindow(m_hWnd) || (m_pCtrlSite != NULL));
