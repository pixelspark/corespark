...
LONG AFXAPI _AfxRecursiveRegDeleteKey(HKEY hParentKey, __in LPTSTR szKeyName)
{
...
{	
	...
	if (SUCCEEDED(LoadTypeLib(strPathNameW.GetString(), &ptlib)))
	{
		...
		if (IsEqualGUID(tlid, tlidActual))
		{
			...
			if (SUCCEEDED(RegisterTypeLib(ptlib,const_cast<LPWSTR>(strPathNameW.GetString()) , 
...
{	
	...
	if (-1 == _stprintf_s(szScratch, _countof(szScratch), _T("CLSID\\%s"), strClassID.GetString()))
	{
		return FALSE;
	}
	if (hkeyClassID.Create(HKEY_CLASSES_ROOT, szScratch, NULL, 0, KEY_ALL_ACCESS, NULL, NULL) != ERROR_SUCCESS)
	{
		return FALSE;
	}
	if (hkeyProgID.Create(HKEY_CLASSES_ROOT, pszProgID, NULL, 0, KEY_ALL_ACCESS, NULL, NULL) !=ERROR_SUCCESS) 
	{
        	return FALSE;
	}
