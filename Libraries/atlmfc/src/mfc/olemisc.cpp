...
BOOL COleException::GetErrorMessage(__out_ecount(nMaxError) LPTSTR lpszError, UINT nMaxError,
	...
	LPSIZEL         lpSizelHim)
{
	...
	if (fIsLink && lpszFullUserTypeName != NULL)
	{
		...
		VERIFY(strLinkedTypeFmt.LoadString(AFX_IDS_PASTELINKEDTYPE));

		int nBuf = ocslen(lpszFullUserTypeName) + strLinkedTypeFmt.GetLength() + 1;
		lpszBuf = (LPOLESTR)::ATL::AtlCoTaskMemCAlloc(nBuf, sizeof(OLECHAR));
		if (lpszBuf != NULL)
		{
			ATL_CRT_ERRORCHECK_SPRINTF(_snwprintf_s(lpszBuf, nBuf, nBuf - 1, strLinkedTypeFmt.GetString(), lpszFullUserTypeName));
