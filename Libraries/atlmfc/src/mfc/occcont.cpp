...
typedef void (*PFNMFCM80ReleaseManagedReferences)(IUnknown*);
...
COleControlContainer::~COleControlContainer()
{
	...
	hr = this->InternalQueryInterface(&iid, (void**)&pIUnknown );
	if( SUCCEEDED( hr ) )
	{
#ifdef _DEBUG
	#ifdef _UNICODE
		const TCHAR szModuleName[] = _T("mfcm80ud.dll");
	#else
		const TCHAR szModuleName[] = _T("mfcm80d.dll");
	#endif
#else
	#ifdef _UNICODE
		const TCHAR szModuleName[] = _T("mfcm80u.dll");
	#else
		const TCHAR szModuleName[] = _T("mfcm80.dll");
	#endif
#endif
		...
		if( NULL != hModule )
		{
			PFNMFCM80ReleaseManagedReferences pfnMFCM80ReleaseManagedReferences = NULL;
			pfnMFCM80ReleaseManagedReferences = (PFNMFCM80ReleaseManagedReferences)GetProcAddress(
																							hModule,
																							"MFCM80ReleaseManagedReferences");

			if( NULL != pfnMFCM80ReleaseManagedReferences )
			{
				pfnMFCM80ReleaseManagedReferences( pIUnknown );
			}
		}
...
int COleControlContainer::GetDlgItemText(int nID, __out_ecount(nMaxCount) LPTSTR lpStr, int nMaxCount) const
{
