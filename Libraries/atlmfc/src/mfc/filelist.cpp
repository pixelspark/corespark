...
AFX_STATIC void AFXAPI _AfxAbbreviateName(__inout_ecount(cchMax) LPTSTR lpszCanon, int cchMax, BOOL bAtLeastName)
{
	...
	if (cchMax < cchFileName)
	{
		if (!bAtLeastName)
			...
			Checked::tcscpy_s(lpszCanon, cchFullPath, lpszFileName);
		return;
	}
	...
	Checked::memcpy_s(lpszCanon + cch, sizeof(_T("\\...")) , _T("\\..."), sizeof(_T("\\...")) );
	Checked::tcscat_s(lpszCanon, cchFullPath, lpszCur);
}
...
void CRecentFileList::Add(LPCTSTR lpszPathName)
{
	...
	if( AfxFullPath(szTemp, lpszPathName) == FALSE )
	{
		ASSERT(FALSE);
		// MFC requires paths with length < _MAX_PATH
		// No other way to handle the error from a void function
