...
{
	ASSERT(m_hWnd == NULL);
	ASSERT(m_pShadowBuffer == NULL || afxData.bWin95 || _AfxGetComCtlVersion()==VERSION_6);
	...
	// characters (not bytes) are in the file.
{
	...
	if (afxData.bWin95 || _AfxGetComCtlVersion()==VERSION_6)
	{
...
LPCTSTR CEditView::LockBuffer() const
{
	...
	if (afxData.bWin95 || _AfxGetComCtlVersion()==VERSION_6)
	{
		// under Win95, it is necessary to maintain a shadow buffer
