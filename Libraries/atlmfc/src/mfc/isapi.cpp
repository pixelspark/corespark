...
namespace ISAPIChecked
{

#if _SECURE_ATL

#define ISAPI_CRT_ERRORCHECK(expr) \
	do { \
		int _nErr = (expr); \
		if (0 != _nErr) \
		{ \
...
inline void __cdecl memcpy_s(void *s1, size_t s1max, const void *s2, size_t n)
{
	ISAPI_CRT_ERRORCHECK(::memcpy_s(s1, s1max, s2, n));
}

#else // !_SECURE_ATL

#define ISAPI_CRT_ERRORCHECK(expr) do { expr; } while (0)
...
extern "C" DWORD WINAPI HttpExtensionProc(EXTENSION_CONTROL_BLOCK *pECB)
{
	...
	{
		ISAPITRACE("Error: unhandled exception caught!\n");
	...
	: m_hFile(hFile)
{
	...
	else
	{
		...
		if (m_pvHeader == NULL)
			throw std::bad_alloc();
		ISAPIChecked::memcpy_s(m_pvHeader, dwHeaderLen+1, pvHeader, dwHeaderLen);
	...
	else
	{
		...
		if (m_pvTail == NULL)
			throw std::bad_alloc();
		ISAPIChecked::memcpy_s(m_pvTail, dwTailLen+1, pvTail, dwTailLen);
	...
	LPVOID pvTail /* = NULL */, DWORD dwTailLen /* = 0 */)
{
	...
	ISAPIENSURE(dwHeaderLen == 0 || pvHeader != NULL);
	ISAPIENSURE(dwTailLen == 0 || pvTail != NULL);
...
LPTSTR CHttpServer::GetQuery(CHttpServerContext* pCtxt, __out LPTSTR lpszQuery)
{
	ISAPIENSURE(pCtxt != NULL);
	ISAPIENSURE(pCtxt->m_pECB != NULL);
	ISAPIENSURE(lpszQuery != NULL);
	// If the request is a POST, get all of the data.  First get what's
	// already available, then read any additional data via the
	// ReadClient() call.

	ISAPIChecked::memcpy_s(lpszQuery, pCtxt->m_pECB->cbAvailable, 
		(LPCTSTR) pCtxt->m_pECB->lpbData, pCtxt->m_pECB->cbAvailable);
	...
	if (pCtxt->m_pECB->cbAvailable < pCtxt->m_pECB->cbTotalBytes)
	{
		...
		if (pstrTarget < lpszQuery)
		{
			ISAPITRACE("Error: Overflow in lpszQuery + pCtxt->m_pECB->cbAvailable!\n");
			return NULL;
		}
		...
		while (cbRemaining > 0)
		{
			...
			if (cbRead == 0)
				break;
			ISAPIENSURE(cbRead <= cbRemaining && cbRead + pstrTarget >= pstrTarget);
			pstrTarget += cbRead;
			cbRemaining -= cbRead;
		}
	}
...
DWORD CHttpServer::HttpExtensionProc(EXTENSION_CONTROL_BLOCK *pECB)
{
    ...
    if (::AfxInvariantStrICmp(pECB->lpszMethod, szGet) == 0)
	{
		pszQuery = pECB->lpszQueryString;
		ctxtCall.m_dwBytesReceived = lstrlen(pszQuery);
	}
	else if (::AfxInvariantStrICmp(pECB->lpszMethod, szPost) == 0)
	{
...
CleanUp:
	...
	// Build the header and content if needed.
	if (pbStream != NULL && ctxtCall.m_bSendHeaders)
	{
		// Check if we need to append \r\n
		const unsigned int nDelimCharCount = 2; // two characters : \r\n
		unsigned int       nDelimsToAdd    = 0;
		DWORD              dwHeaderSize    = ctxtCall.m_dwEndOfHeaders;
		DWORD              dwBufferSize    = 0;

		if( ( ctxtCall.m_dwEndOfHeaders < 2 ) ||
			! ( ( '\r' == pbStream[ctxtCall.m_dwEndOfHeaders - 2] ) && 
				( '\n' == pbStream[ctxtCall.m_dwEndOfHeaders - 1] ) ) )
		{
			nDelimsToAdd = 2;
		}
		else
		{
			if( ( ctxtCall.m_dwEndOfHeaders < 4 ) ||
				! ( ( '\r' == pbStream[ctxtCall.m_dwEndOfHeaders - 4] ) && 
					( '\n' == pbStream[ctxtCall.m_dwEndOfHeaders - 3] ) ) )
			{
				nDelimsToAdd = 1;
			}
		}

		dwBufferSize = dwHeaderSize + ( nDelimsToAdd * nDelimCharCount ) + 1;
		if( dwBufferSize < dwHeaderSize )
		{
			// Wrapping occured.
			pECB->dwHttpStatusCode = HTTP_STATUS_SERVER_ERROR;
			dwRet = HSE_STATUS_ERROR;
#ifdef _DEBUG
			ISAPITRACE1("Error: Header buffer size overflow! Can not ensure header terminates with \\r\\n\\r\\n. Header size is (%d) bytes!\n", dwHeaderSize);
#endif
		}
		else
		{
			pbHeader = new BYTE[ dwBufferSize ];
			if( NULL == pbHeader )
			{
				pECB->dwHttpStatusCode = HTTP_STATUS_SERVER_ERROR;
				dwRet = HSE_STATUS_ERROR;
#ifdef _DEBUG
				ISAPITRACE1("Error: Unable to allocate (%d) bytes for headers!\n", dwBufferSize);
#endif
			}
			else
			{
				memcpy( pbHeader, pbStream, dwHeaderSize );

				for(unsigned int i = 0 ; i < nDelimsToAdd ; ++i )
				{
					pbHeader[ dwHeaderSize++ ] = '\r';
					pbHeader[ dwHeaderSize++ ] = '\n';
				}
				pbHeader[ dwHeaderSize ] = _T('\0');
			}
		}
	...
	else
	{
		if( pbContent != NULL )
		{
			if (!OnWriteBody(&ctxtCall, pbContent, dwSize))
			{
				dwRet = HSE_STATUS_ERROR;
				pECB->dwHttpStatusCode = HTTP_STATUS_SERVER_ERROR;
#ifdef _DEBUG
...
void CHttpServer::BuildStatusCode(__out LPTSTR pszResponse, DWORD dwCode)
{
...
BOOL CHttpServer::OnParseError(CHttpServerContext* pCtxt, int nMethodRet)
{
	...
	if (pCtxt->m_pStream != NULL)
	{
		...
		default:
			...
			//Casting DWORD to string is intentional as AFX_IDS_HTTP_NO_TEXT contains %d and not %s. 
...
#endif
		{
			...
			if (::LoadString(hRes, AFX_IDS_HTTP_TITLE, szTitle, _countof(szTitle)) > 0)
			{
				...
				_stprintf_s(szTitleCopy, _countof(szTitleCopy), szTitle, pCtxt->m_pECB->dwHttpStatusCode); 
			...
			if (pszObject != NULL)
			{
				...
				UINT nLen = lstrlen(szBuffer) + 1;
				nLen +=(nResource == AFX_IDS_HTTP_NO_TEXT) ? AFXISAPI_CHARCOUNT_CONSTANT(INT_MIN) : lstrlen(pszObject);
				TCHAR* pstrFormat = new TCHAR[nLen];
				if (pstrFormat != NULL)
				{
					_stprintf_s(pstrFormat, nLen, szBuffer, pszObject); 
...
{
	ISAPIENSURE(pCtxt != NULL && pCtxt->m_pStream != NULL);
#ifdef _DEBUG
...
int CHttpServer::CallFunction(CHttpServerContext* pCtxt,
	__in LPTSTR pszQuery, __in LPTSTR pszCommand)
{
	ISAPIENSURE(pCtxt != NULL);
	int nRet;
		...
		else
		{
			...
			if (pszHiddenCommand != NULL)
			{
				*pszHiddenCommand = '\0';
				...
				if (::AfxInvariantStrICmp(pszQuery, _T("MfcISAPICommand")) == 0)
				{
		...
		for (iEntry = 0; iEntry < cEntries && pRet == NULL; iEntry++, pCurrent++)
		{
			...
			if (pCmdDefault == NULL)
			{
				if (pszMethod == NULL && pCurrent->pszArgs == NULL)
					...
					&& ::AfxInvariantStrICmp(pszFnName, pszMethod) == 0)
					pRet = pCurrent;
			}
...
UINT PASCAL CHttpServer::GetStackSize(const BYTE* pbParams)
{
	...
	while (*pbParams != 0 && *pbParams != IT_EMPTY)
	{
		...
		ISAPIENSURE(*pbParams >= 1 && *pbParams <= _countof(rgnByValue));
...
int CHttpServer::CallMemberFunc(CHttpServerContext* pCtxt,
	...
	AFX_PARSEMAP_ENTRY* pParams, __in LPTSTR pszParams)
{
	ISAPIASSERT(NULL != pCtxt);
	ISAPIENSURE(NULL != pEntry);
	...
	if (nRet == callOK)
	{
		...
		__try
		{
			if (nSizeArgs + _SCRATCH_SIZE >= nSizeArgs)
			{
...
#endif
{
	...
	*(_STACK_PTR*)pStack = (_STACK_PTR)pCtxt;
	...
	ISAPIChecked::memcpy_s(ppszValues, sizeof(double) * pDefParams->nParams, 
		pDefParams->ppszDefaults, sizeof(double) * pDefParams->nParams);
...
#pragma warning(pop)
			...
			if (pszValue != NULL)
			{
				*pszValue++ = '\0';
				...
				{
					if (::AfxInvariantStrICmp(pDefParams->ppszInfo[nIndex*2], pszCurParam) == 0)
					{
						bFound = TRUE;
						break;
					}
...
LPVOID CHttpServer::PreprocessString(__inout LPTSTR psz)
{
	CHttpArgList::Unescape(psz);
	return (LPVOID) psz;
}

void CHttpArgList::Unescape(__inout LPTSTR psz)
{
	ISAPIENSURE(psz != NULL);
...
bool CHttpArgList::BuildArgList(__inout LPTSTR psz, TCHAR cDelimiter)
{
	ISAPIENSURE(psz != NULL);
	...
	UINT nBytes)
{
	...
	ISAPIChecked::memcpy_s(lpMemTarget, nBytes, lpMemSource, nBytes);
	return lpMemTarget;
}
...
void CHtmlStream::GrowStream(DWORD dwNewLen)
{
	if (dwNewLen > m_nBufferSize)
	{
		...
		ISAPIENSURE(dwNewBufferSize >= dwNewLen);
...
{
	Write(psz, psz != NULL ? lstrlen(psz) : 0);
	return *this;
}
...
{
	TCHAR sz[16];
	int nLen = _stprintf_s(sz, _countof(sz), szDecimalFormat, w);
	Write(sz, nLen);
	return *this;
}
...
{
	TCHAR sz[32];
	int nLen = _stprintf_s(sz, _countof(sz), szDecimalFormat, dw);
	Write(sz, nLen);
	return *this;
}
...
{
	TCHAR sz[64];
	int nLen = _stprintf_s(sz, _countof(sz), szFloatFormat, f);
	Write(sz, nLen);
	return *this;
}
...
{
	TCHAR sz[64];
	int nLen = _stprintf_s(sz, _countof(sz), szFloatFormat, d);
	Write(sz, nLen);
	return *this;
}
...
void CHtmlStream::Write(const void* lpBuf, UINT nCount)
{
	if (nCount == 0)
		return;
	ISAPIENSURE(lpBuf != NULL);

	UINT nUpperBound = m_nPosition + nCount;

	if (nUpperBound < m_nPosition || nUpperBound < nCount)
		return;
	if (nUpperBound > m_nBufferSize)
		GrowStream(nUpperBound);

	ISAPIASSERT(nUpperBound <= m_nBufferSize);
