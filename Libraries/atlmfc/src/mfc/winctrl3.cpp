...
bool CCheckListBox::PreDrawItemThemed(CDC* pDC, DRAWITEMSTRUCT &drawItem, int nCheck, int cyItem)
{
	...
	if (_ThemeHelper::IsAppThemed())
	{
		...
		if (hTheme != NULL)
		{
			...
			if (SUCCEEDED(_ThemeHelper::GetThemePartSize(hTheme, pDC->m_hDC, BP_CHECKBOX, nState, NULL, TS_TRUE, &size)))
			{
				...
				if (SUCCEEDED(_ThemeHelper::DrawThemeBackground(hTheme, pDC->m_hDC, BP_CHECKBOX, nState, &rectCheckBox, NULL)))
				{
					bRet = true;
					drawItem.rcItem.left = drawItem.rcItem.left + size.cx + 3;
				}
				_ThemeHelper::CloseThemeData(hTheme);
			}
		}
	}
	return bRet;
