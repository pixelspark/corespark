...
int AFXAPI AfxLoadString(UINT nID, __out_ecount_opt(nMaxBuf) LPWSTR lpszBuf, UINT nMaxBuf)
{
...
int AFXAPI AfxLoadString(UINT nID, __out_ecount_opt(nMaxBuf) LPSTR lpszBuf, UINT nMaxBuf)
{
...
BOOL WINAPI DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID)
{
	if (dwReason == DLL_PROCESS_ATTACH)
	{
		...
		HINSTANCE hLangDLL = AfxLoadLangResourceDLL(_T("MFC80%s.DLL"));
