...
BOOL CReBarCtrl::GetColorScheme(COLORSCHEME* lpcs)
{
   ...
   //IA64: Returns non-zero if successful, so can't just truncate to 32 bits
	return (SendMessage(RB_GETCOLORSCHEME, 0, (LPARAM)lpcs) != 0);
}
...
BOOL CListCtrl::SetBkImage(__in LPTSTR pszUrl, BOOL fTile /*= TRUE*/, int xOffsetPercent /*= 0*/, int yOffsetPercent /*= 0*/)
{
	LVBKIMAGE lv;
...
BOOL CHeaderCtrl::GetOrderArray(LPINT piArray, int iCount)
{
	ASSERT(::IsWindow(m_hWnd));
