...
SCODE _AfxParseDisplayName(LPMONIKER lpmk, LPBC lpbc, __in_z LPTSTR lpszRemainder,
...
STDMETHODIMP COleUILinkInfo::SetLinkSource(
	DWORD dwLink, __in_ecount(lenFileName) LPTSTR lpszDisplayName, ULONG lenFileName,
...
STDMETHODIMP COleUILinkInfo::GetLinkSource(
	DWORD dwLink, __deref_out LPTSTR* lplpszDisplayName, ULONG* lplenFileName,
	__deref_out LPTSTR* lplpszFullLinkType, __deref_out LPTSTR* lplpszShortLinkType,
...
void COlePasteSpecialDialog::AddFormat(const FORMATETC& formatEtc,
	__in LPTSTR lpszFormat, __in LPTSTR lpszResult, DWORD dwFlags)
{
	ASSERT_VALID(this);
