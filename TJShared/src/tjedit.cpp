...
EditWnd::EditWnd(HWND parent): ChildWnd(L"", parent, false, false) {
	...
	_ctrl = CreateWindowEx(0, L"EDIT", L"", WS_CHILD|WS_TABSTOP|ES_AUTOHSCROLL, 0, 0, 10, 10, _wnd, 0, GetModuleHandle(NULL), 0);
...
void EditWnd::Layout() {
	...
	int vMargin = (rc.bottom-rc.top-ControlHeight)/2;
	SetWindowPos(_ctrl, 0, rc.left+2, rc.top+vMargin, rc.right-rc.left-4, ControlHeight, SWP_NOZORDER);
}
