...
BrowserWnd::BrowserWnd(HWND w, std::wstring title): ChildWnd(title.c_str(), w, true, false) {
	...
	_tools = GC::Hold(new BrowserToolbarWnd(this, _wnd));

	SetStyle(WS_CLIPCHILDREN);
	SetStyleEx(WS_EX_CONTROLPARENT);
	RECT rc = {0,0,100,100};

	if(((CAxWindow*)_ax)->Create(_wnd, rc,_T("Mozilla.Browser.1"),WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN,0)) {
		Log::Write(L"BrowserWnd/Constructor", L"Using Mozilla/Gecko");
	}
	else if(((CAxWindow*)_ax)->Create(_wnd, rc,_T("Shell.Explorer.2"),WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN,0)) {
...
void BrowserWnd::Layout() {
	tj::shared::Rectangle r = GetClientRectangle();
	...
	if(((CAxWindow*)_ax)->IsWindow()) {
		((CAxWindow*)_ax)->SetParent(_wnd);
		((CAxWindow*)_ax)->MoveWindow(r.GetLeft(),r.GetTop(), r.GetWidth(), r.GetHeight(), TRUE);
	}
}

LRESULT BrowserWnd::Message(UINT msg, WPARAM wp, LPARAM lp) {
	if(msg==WM_SIZE) {
		Layout();
	}
...
BrowserToolbarWnd::BrowserToolbarWnd(tj::shared::BrowserWnd *browser, HWND parent): ToolbarWnd(parent) {
	_browser = browser;
...
void BrowserToolbarWnd::Layout() {
	RECT rc;
	GetClientRect(_wnd, &rc);
	_url->Move(5*24+2, 2, rc.right-5*24-6,20);
	ToolbarWnd::Layout();
}
